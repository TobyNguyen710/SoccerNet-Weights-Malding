...
class MLLegibilityClassifier:
    """419-style ML legibility classifier using a ResNet34 checkpoint."""

    def __init__(self, model_path: str, device: Optional[str] = None, threshold: float = 0.85):
    ...
    def _load_model(self) -> nn.Module:
        ...
        for key, value in state_dict.items():
            candidates = [
                key,
                key.replace('model_ft.', 'backbone.'),
                key.replace('model.', 'backbone.'),
            ]
            if not any(key.startswith(p) for p in ('model_ft.', 'model.', 'backbone.')):
    ...
    def classify(self, img_path: str) -> Dict:
        try:
            ...
            return {
                ...
                'score': score,
                'metrics': {'model_score': score},
            }
        except Exception as e:
            logger.warning(f"ML legibility error on {img_path}: {e}")
            return {
                'image': img_path,
                'legible': False,
                'status': LegibilityStatus.UNKNOWN.value,
...
class LegibilityClassifierPipeline:
    """Use only ML backend and fail fast on missing/invalid checkpoint."""

    def __init__(self, model_path: Optional[str], threshold: float = 0.85, device: Optional[str] = None):
        self.backend = 'ml'
        self.device = _resolve_device(device)
        self.threshold = threshold

        if not model_path:
            raise ValueError("Legibility model path is required for ML-only mode.")

        model_path_obj = Path(model_path)
        if not model_path_obj.exists():
            raise FileNotFoundError(f"Legibility model checkpoint not found: {model_path_obj}")

        try:
            self.classifier = MLLegibilityClassifier(model_path=str(model_path_obj), device=self.device, threshold=threshold)
        except Exception as e:
            raise RuntimeError(f"Failed to initialize ML legibility backend from {model_path_obj}: {e}") from e

        logger.info(f"Legibility backend: {self.backend} (device={self.device}, threshold={self.threshold})")
...
def train_parseq(dataset_root: Path, epochs: int, output_dir: Path) -> str:
    """
    ...
    cmd = [
        ...
        "charset=10_digits",          # digit-only vocab: 10 chars + EOS/BOS/PAD
        "model.max_label_length=2",   # jersey numbers are at most 2 digits
        "data.max_label_length=2",
        f"model.batch_size=128",      # larger batch → more stable gradients
...
def run_legibility_only(
    tracklets: List[Tuple[Path, Optional[str]]],
    legibility_classifier,
    output_dir: Path,
    filter_by_legibility: bool,
) -> None:
    """Run only legibility scoring and write outputs, without LMDB creation or PARSeq training."""
    output_dir.mkdir(parents=True, exist_ok=True)
    kept_samples, legibility_data, stats = collect_samples(
        tracklets,
        legibility_classifier=legibility_classifier,
        filter_by_legibility=filter_by_legibility,
    )

    scores_path = output_dir / "legibility_scores.json"
    with open(scores_path, 'w', encoding='utf-8') as f:
        json.dump(legibility_data, f, indent=2)

    kept_path = output_dir / "legibility_kept_samples.txt"
    with open(kept_path, 'w', encoding='utf-8') as f:
        for img_path, label in kept_samples:
            f.write(f"{img_path}\t{label}\n")

    summary_path = output_dir / "legibility_summary.json"
    with open(summary_path, 'w', encoding='utf-8') as f:
        json.dump(stats, f, indent=2)

    logger.info("Legibility-only mode complete")
    logger.info(f"  Scores: {scores_path}")
    logger.info(f"  Kept samples: {kept_path}")
...
def main():
    ...
    parser.add_argument("--only-legibility", action="store_true",
                       help="Run only legibility classifier and stop before LMDB/training")
    parser.add_argument("--legibility-model-path", type=Path, default=Path("./models/legibility_resnet34_soccer_20240215.pth"),
                       help="Path to ResNet34 legibility checkpoint (419 style)")
    parser.add_argument("--legibility-threshold", type=float, default=0.85,
                       help="Legibility score threshold for ML backend")
    ...
    parser.add_argument("--val-fraction", type=float, default=0.1,
                       help="Fraction of samples for validation split (default: 0.1)")
    ...
    logger.info(f"Only legibility mode: {args.only_legibility}")
    ...
    if args.skip_legibility:
        legibility_classifier = None
    else:
        legibility_model_path = project_root / args.legibility_model_path if not args.legibility_model_path.is_absolute() else args.legibility_model_path
        legibility_classifier = LegibilityClassifierPipeline(
            model_path=str(legibility_model_path),
            threshold=args.legibility_threshold,
            device='cuda' if torch.cuda.is_available() else 'cpu',
        )

    if args.only_legibility:
        if legibility_classifier is None:
            raise ValueError("--only-legibility requires legibility classifier enabled. Remove --skip-legibility.")
        logger.info("\n[Step 3] Running legibility-only mode...")
        run_legibility_only(
            tracklets=tracklets,
            legibility_classifier=legibility_classifier,
    ...
    try:
        checkpoint = train_parseq(dataset_root, args.epochs, args.output_dir)
        logger.info(f"\n✅ Training completed!")
