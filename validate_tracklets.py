...
def predict_on_image(model: nn.Module, image_path: str, img_transform, max_pred_len: int = 2) -> tuple[str, float]:
    """
    Run inference on a single image.
    
    Returns:
        (Predicted text string, confidence score)
    """
    try:
        ...
        with torch.no_grad():
            ...
            pred, conf = decode_logits(logits, model, max_pred_len=max_pred_len)
...
def decode_logits(logits, model, max_pred_len: int = 2) -> tuple[str, float]:
    ...
    conf_mean = float(conf[0].mean().item()) if conf else 0.0
...
def validate(
    ...
    for img_path, gt_label in tqdm(val_samples, desc="Validating"):
        # Predict
        pred, pred_conf = predict_on_image(model, img_path, img_transform, max_pred_len=max_pred_len)
